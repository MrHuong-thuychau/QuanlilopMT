...
function stats(){
  ...
  const talent=ss.filter(s=>scoreTypes().some(k=>typeof getScores(s.id)[k]==="number"&&getScores(s.id)[k]>=9));
...
function renderOverview(){
  ...
  <div class="cards overview-cards">
    ...
    <div class="card"><span class="muted">Học sinh có năng khiếu</span><div class="metric">${d.talent.length}</div><span class="muted">Có ít nhất 1 điểm ≥ 9</span></div>
  </div>
...
scores:()=>{if(!curClass())return noClass();let ss=classStudents();return `<div class="page"><div class="section"><div class="section-head"><div><h2>🎨 Sổ điểm Mĩ thuật — ${esc(curClass().name)}</h2><div class="muted">TX1, TX2, TX3 • Giữa kỳ • Cuối kỳ</div></div><div><button class="btn secondary" onclick="manageComments()">📝 Mẫu nhận xét</button> <button class="btn primary" onclick="showScore()">＋ Nhập điểm</button></div></div>
<div class="notice"><b>Quy ước:</b> nhập <b>CĐ</b> hoặc điểm <b>5 / 5,5 / 6 / 6,5 … 10</b>. Điểm dưới 5 đánh giá là CĐ. Nhấn Enter hoặc rời ô để lưu.</div>
...
<td><select class="mini-select" onchange="quickFinal('${s.id}',this.value)"><option value="">—</option><option value="Đạt" ${finalResult(s.id)==="Đạt"?"selected":""}>Đạt</option><option value="Chưa đạt" ${finalResult(s.id)==="Chưa đạt"?"selected":""}>Chưa đạt</option></select></td>
...
reports:()=>{if(!curClass())return noClass();let ss=classStudents(),z=ss.map(s=>resultFor(s.id));return `<div class="page"><div class="section"><div class="section-head"><h2>📑 Báo cáo — ${esc(curClass().name)}</h2><button class="btn primary" onclick="exportClassExcel()">⇩ Xuất Excel</button></div><div class="cards"><div class="card"><span class="muted">Sĩ số</span><div class="metric">${ss.length}</div></div><div class="card"><span class="muted">Đạt</span><div class="metric">${z.filter(x=>x==="Đạt").length}</div></div><div class="card"><span class="muted">Chưa đạt</span><div class="metric">${z.filter(x=>x==="Chưa đạt").length}</div></div><div class="card"><span class="muted">Chuyên cần</span><div class="metric">${attendanceRate()}%</div></div></div><div class="notice">Báo cáo được tính riêng cho lớp đang chọn. Có thể xuất Excel để lưu hồ sơ hoặc xử lý tiếp.</div></div></div>`},
settings:()=>`<div class="page"><div class="section"><div class="section-head"><h2>⚙️ Cài đặt</h2><button class="btn primary" onclick="saveSettings()">Lưu</button></div><div class="formgrid"><div class="field"><label>Giáo viên</label><input id="setTeacher" class="input" value="${esc(state.settings.teacher)}"></div><div class="field"><label>Trường</label><input id="setSchool" class="input" value="${esc(state.settings.school)}"></div><div class="field"><label>Năm học</label><input id="setYear" class="input" value="${esc(state.settings.schoolYear)}"></div></div></div>
<div class="section"><h2>💾 Dữ liệu</h2><p class="muted">Dữ liệu phiên bản này lưu trong trình duyệt. Hãy sao lưu định kỳ.</p><button class="btn secondary" onclick="exportBackup()">Xuất bản sao JSON</button> <button class="btn danger" onclick="wipeAll()">Xóa toàn bộ dữ liệu</button></div></div>`
};
...
function scoreCell(id,type,v){
  ...
  return `<td class="score-cell ${cls}"><div class="score-edit"><input class="score-input" type="text" inputmode="decimal" value="${esc(value)}" placeholder="CĐ/5–10"
    ...
    onblur="quickScore('${id}','${type}',this.value)"><button type="button" class="cd-btn ${value==="CĐ"?"active":""}" onclick="quickScore('${id}','${type}','CĐ')">CĐ</button></div></td>`;
}
...
function quickFinal(id,value){setFinal(id,value);save();render();toast("Đã cập nhật xếp loại")}
...
function exportBackup(){let a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify(state,null,2)],{type:"application/json"}));a.download="Thuy_Chau_Art_Class_Manager_Backup.json";a.click()}
