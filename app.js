...
let state=load();state.settings=state.settings||{};if(!state.settings.cloudUrl)state.settings.cloudUrl=V5_GS_URL;if(!state.settings.cloudKey)state.settings.cloudKey=V5_GS_KEY;
let currentPage="dashboard",dragId=null,syncTimer=null,syncPollTimer=null,syncBusy=false;
function clone(x){return JSON.parse(JSON.stringify(x))}
function hashable(x){try{return JSON.stringify(x)}catch(e){return ""}}
function syncMetaLoad(){try{return JSON.parse(localStorage.getItem(SYNC_META_KEY)||"{}")||{}}catch(e){return {}}}
function syncMetaSave(m){localStorage.setItem(SYNC_META_KEY,JSON.stringify(m))}
function syncSettingsValue(x){return {teacher:x?.teacher||"",school:x?.school||"",schoolYear:x?.schoolYear||""}}
function prepareSyncMeta(previous,next,now){
  const m=syncMetaLoad();
  m.collections=m.collections||{};
  SYNC_COLLECTIONS.forEach(k=>{
    if(!m.collections[k])m.collections[k]={};
    const oldArr=Array.isArray(previous?.[k])?previous[k]:[];
    const newArr=Array.isArray(next?.[k])?next[k]:[];
    const keyed=oldArr.some(x=>x&&x.id!=null)||newArr.some(x=>x&&x.id!=null);
    if(keyed){
      const oldBy=new Map(oldArr.filter(x=>x&&x.id!=null).map(x=>[String(x.id),hashable(x)]));
      const newBy=new Map(newArr.filter(x=>x&&x.id!=null).map(x=>[String(x.id),x]));
      newBy.forEach((x,id)=>{if(!oldBy.has(id)||oldBy.get(id)!==hashable(x))m.collections[k][id]=now});
      oldBy.forEach((_,id)=>{if(!newBy.has(id))m.collections[k][id]={deletedAt:now}});
    }else if(hashable(oldArr)!==hashable(newArr)){m.collections[k].__whole=now}
  });
  if(hashable(syncSettingsValue(previous?.settings))!==hashable(syncSettingsValue(next?.settings)))m.settingsUpdatedAt=now;
  return m;
}
function load(){
  try{
    const raw=JSON.parse(localStorage.getItem(KEY)||"{}");
    const x={...structuredClone(DEFAULT),...raw};
    x.scores=Array.isArray(x.scores)?x.scores:[]; x.comments=Array.isArray(x.comments)?x.comments:[];
    ...
    const meta=x._syncMeta||{};
    const metaCollections=meta.collections||{};
    const hasMeta=SYNC_COLLECTIONS.some(k=>{const z=metaCollections[k]||{};return Object.keys(z).length>0}) || !!meta.settingsUpdatedAt;
    if(!hasMeta){
      const at=x._localUpdatedAt||x._lastSaved||new Date().toISOString(), m={collections:{},settingsUpdatedAt:at};
      SYNC_COLLECTIONS.forEach(k=>{m.collections[k]={};const arr=Array.isArray(x[k])?x[k]:[];if(arr.some(v=>v&&v.id!=null))arr.forEach(v=>{if(v&&v.id!=null)m.collections[k][String(v.id)]=at});else m.collections[k].__whole=at});
      x._syncMeta=m;
      localStorage.setItem(SYNC_META_KEY,JSON.stringify(m));
      localStorage.setItem(KEY+"_shadow",JSON.stringify(x));
    }
    return x;
...
function save(opts={}){
  ...
  state._localUpdatedAt=state._lastSaved;
  const oldShadow=localStorage.getItem(KEY+"_shadow");
  let prevShadow={};try{prevShadow=oldShadow?JSON.parse(oldShadow):{}}catch(e){}
  state._syncMeta=prepareSyncMeta(prevShadow,state,state._lastSaved);
  localStorage.setItem(KEY,JSON.stringify(state));
  localStorage.setItem(KEY+"_shadow",JSON.stringify(state));
  syncMetaSave(state._syncMeta);
...
function renderSettingsPage(){
  ...
  const auto=!!state.settings.autoSync;
  ...
  <div class="section"><h2>☁️ Đồng bộ dữ liệu giữa các máy</h2><p class="muted">Dữ liệu vẫn lưu cục bộ. Khi cấu hình Google Apps Script bên dưới, anh có thể đồng bộ cùng một bộ dữ liệu trên máy ở nhà, máy ở trường và điện thoại.</p>
    ...
    <div class="notice"><b>V4.0.3 — Google Sheets V5 là dữ liệu trung tâm:</b> Lần đầu hãy bấm <b>🟦 Lấy dữ liệu V5 làm dữ liệu nền</b>. Thao tác này thay dữ liệu cục bộ bằng đúng dữ liệu đang có trên Google Sheets V5 (16 lớp/655 HS), rồi đánh dấu máy đã nối cầu. Sau đó dùng <b>🔄 Đồng bộ hai chiều</b>. V4 không tự lấy dữ liệu cũ trên máy đè lên V5 khi chưa hoàn tất bước nối cầu.</div>
  </div>
...
function downloadTemplate(){let rows=[["STT","Họ và tên","Giới tính","Ngày sinh","SĐT phụ huynh","Tổ","Lớp"],[1,"Nguyễn Văn A","Nam","","","1","6/1"],[2,"Trần Thị B","Nữ","","","1","6/1"]];let ws=XLSX.utils.aoa_to_sheet(rows),wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"DanhSach");XLSX.writeFile(wb,"Mau_DanhSach_HocSinh_Thuy_Chau.xlsx")}
function exportClassExcel(){let ss=classStudents(),rows=[["STT","Họ và tên","Giới tính","Tổ","TX1","TX2","TX3","Giữa kỳ","Cuối kỳ","Xếp loại cuối kỳ","Nhận xét cuối kỳ"]];ss.forEach((s,i)=>{let z=getScores(s.id);rows.push([i+1,s.name,s.gender,s.group,z.TX1??"",z.TX2??"",z.TX3??"",z.GK??"",z.CK??"",finalResult(s.id),getComment(s.id)])});let ws=XLSX.utils.aoa_to_sheet(rows);ws["!cols"]=[{wch:6},{wch:28},{wch:10},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10},{wch:10},{wch:18},{wch:42}];ws["!autofilter"]={ref:ws["!ref"]};let wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,curClass().name);XLSX.writeFile(wb,`SoDiem_MiThuat_${curClass().name.replace("/","-")}.xlsx`)}
...
function cloudConfig(){return {url:String(state.settings.cloudUrl||V5_GS_URL).trim().replace(/\/+$/,""),key:String(state.settings.cloudKey||V5_GS_KEY).trim()}}
...
function saveCloudSettings(){
  ...
  state.settings.autoSync=!!document.querySelector("#autoSync")?.checked;
  save({skipSync:true}); startCloudAutoSync(); render(); toast("Đã lưu cấu hình đồng bộ");
}
function scheduleAutoSync(){
  if(!state.settings?.autoSync||!state.settings?.cloudUrl||!state.settings?.cloudKey)return;
  clearTimeout(syncTimer); syncTimer=setTimeout(()=>syncCloud(true),2200);
}
function cloudSnapshot(){
  const x=clone(state);
  x._syncMeta=state._syncMeta||syncMetaLoad();
  delete x._cloudStatus; delete x._cloudUpdatedAt; delete x._cloudVersion; delete x._cloudPublishId; delete x._cloudChecksum;
  return x;
}
...
function cloudForm(){
  let f=document.getElementById("tcCloudPostForm"); if(f)return f;
...
function cloudGet(done,silent=false){
  const {url,key}=cloudConfig();
  if(!validCloudUrl(url)||!key){if(!silent)toast("Hãy cấu hình URL Web App và mã đồng bộ trước.");done&&done(null);return false}
...
function v4Hash(x){
  let h=2166136261>>>0, str=hashable(x)||"";
  for(let i=0;i<str.length;i++){h^=str.charCodeAt(i);h=Math.imul(h,16777619)>>>0}
  return h.toString(16).padStart(8,"0");
}
function v4Comparable(x){
  const y=clone(x||{});
  delete y._cloudStatus;delete y._cloudUpdatedAt;delete y._cloudVersion;delete y._cloudPublishId;delete y._cloudChecksum;delete y._syncPushId;delete y._syncPushStartedAt;
  if(y.settings)y.settings={teacher:y.settings.teacher||"",school:y.settings.school||"",schoolYear:y.settings.schoolYear||""};
  return y;
}
...
function applyCloudState(data,status,meta={}){
  ...
  if(meta.updatedAt)state._cloudUpdatedAt=meta.updatedAt;
  if(meta.version!==undefined)state._cloudVersion=meta.version;
  if(meta.publishId!==undefined)state._cloudPublishId=meta.publishId;
  if(meta.checksum!==undefined)state._cloudChecksum=meta.checksum;
  state._cloudStatus=status||"Đã đồng bộ";
  localStorage.setItem(KEY,JSON.stringify(state));localStorage.setItem(KEY+"_shadow",JSON.stringify(state));syncMetaSave(state._syncMeta||{});render();
}
...
function pushCloud(silent=false,baseVersion=null){
  const {url,key}=cloudConfig();if(!validCloudUrl(url)||!key){if(!silent)toast("Hãy cấu hình URL Web App và mã đồng bộ trước.");return false}
  if(syncBusy)return false;syncBusy=true;
  const payload=cloudSnapshot();payload._syncPushId=crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random();payload._syncPushStartedAt=new Date().toISOString();payload._baseVersion=baseVersion!==null?baseVersion:(state._cloudVersion||0);
  const expected=v4Fingerprint(payload);state._cloudStatus="Đang đồng bộ dữ liệu…";localStorage.setItem(KEY,JSON.stringify(state));if(!silent)render();
  postCloud("push",payload,()=>{setTimeout(()=>cloudGet(j=>{
    if(j?.conflict){syncBusy=false;state._cloudStatus="⚠️ Cloud đã thay đổi — cần tải bản mới rồi đồng bộ lại";localStorage.setItem(KEY,JSON.stringify(state));render();if(!silent)toast("⚠️ Có thay đổi mới trên Cloud. Em không ghi đè dữ liệu đó.");return}
    if(j?.data&&v4Fingerprint(j.data)===expected){syncBusy=false;applyCloudState(j.data,"☁️ Đã đồng bộ và xác nhận",{updatedAt:j.updatedAt,version:j.version,publishId:j.publishId,checksum:j.checksum});if(!silent)toast("☁️ Đồng bộ thành công — "+localStatsText(j.data));return}
    syncBusy=false;state._cloudStatus="⚠️ Cloud chưa khớp dữ liệu vừa gửi";localStorage.setItem(KEY,JSON.stringify(state));render();if(!silent)toast("⚠️ Không xác nhận được dữ liệu sau khi đồng bộ.");
  },true),900)});
  return true;
}
function mergeCloudData(cloud){
  const local=clone(state), cm=cloud?cloud._syncMeta||{}:{}, lm=local._syncMeta||syncMetaLoad(), merged=clone(local);
  SYNC_COLLECTIONS.forEach(k=>{
    const lArr=Array.isArray(local[k])?local[k]:[], cArr=Array.isArray(cloud?.[k])?cloud[k]:[];
    const keyed=lArr.some(x=>x&&x.id!=null)||cArr.some(x=>x&&x.id!=null);
    if(!keyed){const lt=lm.collections?.[k]?.__whole||"",ct=cm.collections?.[k]?.__whole||"";merged[k]=ct>lt?clone(cArr):clone(lArr);return}
    const lBy=new Map(lArr.filter(x=>x&&x.id!=null).map(x=>[String(x.id),x])),cBy=new Map(cArr.filter(x=>x&&x.id!=null).map(x=>[String(x.id),x]));
    const lmK=lm.collections?.[k]||{},cmK=cm.collections?.[k]||{},ids=new Set([...lBy.keys(),...cBy.keys(),...Object.keys(lmK),...Object.keys(cmK)]),out=[];
    ids.forEach(id=>{const lt=typeof lmK[id]==="string"?lmK[id]:(lmK[id]?.deletedAt||""),ct=typeof cmK[id]==="string"?cmK[id]:(cmK[id]?.deletedAt||""),winner=ct>lt?"cloud":lt>ct?"local":(cBy.has(id)?"cloud":"local"),del=winner==="cloud"?(!!cmK[id]?.deletedAt):(!!lmK[id]?.deletedAt),item=winner==="cloud"?cBy.get(id):lBy.get(id);if(!del&&item)out.push(clone(item))});
    merged[k]=out;
  });
  const ls=lm.settingsUpdatedAt||"",cs=cm.settingsUpdatedAt||"";merged.settings=cs>ls?{...clone(local.settings||{}),...syncSettingsValue(cloud.settings)}:{...clone(local.settings||{})};
  merged._syncMeta={collections:{}};SYNC_COLLECTIONS.forEach(k=>{const a=cm.collections?.[k]||{},b=lm.collections?.[k]||{},o={...a,...b};Object.keys(o).forEach(id=>{const av=a[id],bv=b[id],at=typeof av==="string"?av:(av?.deletedAt||""),bt=typeof bv==="string"?bv:(bv?.deletedAt||"");if(av!==undefined||bv!==undefined){if(at>bt)o[id]=av;else if(bt>at)o[id]=bv;else if(av?.deletedAt||bv?.deletedAt)o[id]=av?.deletedAt?av:bv}});merged._syncMeta.collections[k]=o});
  merged._syncMeta.settingsUpdatedAt=cs>ls?cs:ls;const clt=cloud?String(cloud._localUpdatedAt||""):"";const llt=String(local._localUpdatedAt||"");merged._localUpdatedAt=llt>clt?llt:clt;return merged;
}
function bridgeReady(){try{return JSON.parse(localStorage.getItem(V5_BRIDGE_FLAG)||"{}").ready===true}catch(e){return localStorage.getItem(V5_BRIDGE_FLAG)==="1"}}
function markBridgeReady(meta){localStorage.setItem(V5_BRIDGE_FLAG,JSON.stringify({ready:true,version:Number(meta?.version||0),publishId:String(meta?.publishId||""),at:new Date().toISOString()}))}
function bootstrapFromV5(){
  if(syncBusy)return;
  const {url,key}=cloudConfig();
  if(!validCloudUrl(url)||!key){toast("Hãy cấu hình URL Web App V5 và mã đồng bộ trước.");return}
  if(!confirm("LẤY DỮ LIỆU V5 LÀM DỮ LIỆU NỀN CHO V4?\n\nDữ liệu hiện có trong trình duyệt V4 sẽ được thay bằng dữ liệu đang có trên Google Sheets V5. Dữ liệu V5 không bị thay đổi."))return;
  syncBusy=true; state._cloudStatus="⏳ Đang lấy dữ liệu chuẩn từ V5…"; localStorage.setItem(KEY,JSON.stringify(state)); render();
  cloudGet(j=>{
    if(!j?.data){syncBusy=false;state._cloudStatus="❌ Không lấy được dữ liệu V5";localStorage.setItem(KEY,JSON.stringify(state));render();toast(j?.error||"V5 chưa trả dữ liệu.");return}
    try{
      const keep=cloudKeepSettings(), data=clone(j.data);
      if(!Array.isArray(data.classes)||!Array.isArray(data.students))throw new Error("Dữ liệu V5 không có classes/students hợp lệ.");
      state={...structuredClone(DEFAULT),...data}; state.settings={...state.settings,...keep};
      state._cloudVersion=Number(j.version||0);state._cloudUpdatedAt=j.updatedAt||"";state._cloudPublishId=j.publishId||"";state._cloudChecksum=j.checksum||"";state._cloudStatus="✅ Đã nối V4 với dữ liệu trung tâm V5";
      localStorage.setItem(KEY,JSON.stringify(state));localStorage.setItem(KEY+"_shadow",JSON.stringify(state));syncMetaSave(state._syncMeta||{});markBridgeReady(j);syncBusy=false;render();toast("🟦 Đã lấy dữ liệu V5 — "+localStatsText(state));
    }catch(e){syncBusy=false;state._cloudStatus="❌ Dữ liệu V5 không hợp lệ: "+e.message;localStorage.setItem(KEY,JSON.stringify(state));render();toast("❌ "+e.message)}
  },false);
}
function syncCloud(silent=false){
  if(syncBusy)return;
  if(!bridgeReady()){ if(!silent) bootstrapFromV5(); return; }
  syncBusy=true;
  cloudGet(j=>{
    if(!j?.data){syncBusy=false;if(!silent)toast(j?"☁️ V5 chưa có dữ liệu.":"❌ Không lấy được dữ liệu V5.");return}
    const cloud=clone(j.data); cloud._cloudVersion=j.version||0;
    const merged=mergeCloudData(cloud);
    const changed=v4Fingerprint(merged)!==v4Fingerprint(cloud);
    applyCloudState(merged,changed?"Đã hợp nhất local + V5, đang ghi nhận…":"☁️ Hai bên đã giống nhau",{updatedAt:j.updatedAt,version:j.version,publishId:j.publishId,checksum:j.checksum});
    markBridgeReady(j); syncBusy=false;
    if(changed)setTimeout(()=>pushCloud(silent,j.version||0),250);else if(!silent)toast("🔄 Dữ liệu đã đồng nhất — "+localStatsText(merged));
  },silent);
}
function startCloudAutoSync(){clearInterval(syncPollTimer);if(!state.settings?.autoSync||!state.settings?.cloudUrl||!state.settings?.cloudKey)return;syncPollTimer=setInterval(()=>{if(document.hidden||syncBusy)return;syncCloud(true)},15000)}
function initializeCloud(){publishCloud(false)}
function loadCloudAsNewDevice(){
  const {url,key}=cloudConfig();if(!validCloudUrl(url)||!key)return toast("Hãy lưu URL Web App và mã đồng bộ trước.");
  if(!confirm("TẢI NGUYÊN TRẠNG DỮ LIỆU CLOUD VỀ MÁY NÀY?\n\nDữ liệu hiện có trên máy này sẽ bị thay thế. Hãy xuất bản sao JSON trước nếu máy này đang có dữ liệu cần giữ."))return;
  if(syncBusy)return;syncBusy=true;cloudGet(j=>{try{if(!j?.data){toast(j?"☁️ Cloud chưa có dữ liệu.":"❌ Không lấy được dữ liệu Cloud.");return}applyCloudState(j.data,"✅ Máy này đã được thiết lập đúng theo V5",{updatedAt:j.updatedAt,version:j.version,publishId:j.publishId,checksum:j.checksum});markBridgeReady(j);toast("📥 Đã tải nguyên trạng V5 — "+localStatsText(j.data))}finally{syncBusy=false}},false);
}
